...
// Funciones para mostrar datos en tablas
function mostrarConductores(conductores) {
  ...
  conductores.forEach(conductor => {
      ...
      tr.innerHTML = `
          ...
          <td>${formatearFecha(conductor.vencimientoLic)}</td>
          <td>${conductor.categoriaLic || ''}</td>
          <td>${conductor.vehiculo ? conductor.vehiculo.patente : '-'}</td>
          <td>
              <button class="btn-edit" onclick="editarConductor('${conductor.dni}')">Editar</button>
              <button class="btn-delete" onclick="eliminarConductor('${conductor.dni}')">Eliminar</button>
          </td>
      `;
...
function mostrarPasajeros(pasajeros) {
  ...
  pasajeros.forEach(pasajero => {
      ...
      tr.innerHTML = `
          ...
          <td>${pasajero.codigoPostal || ''}</td>
          <td>${pasajero.domicilio || ''}</td>
          <td>${pasajero.vehiculoasignado ? pasajero.vehiculoasignado.patente : '-'}</td>
          <td>
              <button class="btn-edit" onclick="editarPasajero('${pasajero.dni}')">Editar</button>
              <button class="btn-delete" onclick="eliminarPasajero('${pasajero.dni}')">Eliminar</button>
          </td>
      `;
      tbody.appendChild(tr);
  });
...
function mostrarVehiculos(vehiculos) {
  ...
  vehiculos.forEach(vehiculo => {
      ...
      tr.innerHTML = `
          ...
          <td>${vehiculo.equipamiento || '-'}</td>
          <td>
              <button class="btn-edit" onclick="editarVehiculo('${vehiculo.patente}')">Editar</button>
              <button class="btn-delete" onclick="eliminarVehiculo('${vehiculo.patente}')">Eliminar</button>
          </td>
      `;
      tbody.appendChild(tr);
  });
...
async function buscarConductor() {
  const searchTerm = document.getElementById('busquedaConductor').value.trim();
  
  if (!searchTerm) {
      const conductores = await window.api.obtenerConductores();
      mostrarConductores(conductores);
      return;
  }
  
  const resultados = await window.api.buscarConductores(searchTerm);
  mostrarConductores(resultados);
}

async function buscarPasajero() {
  const searchTerm = document.getElementById('busquedaPasajero').value.trim();
  
  if (!searchTerm) {
      const pasajeros = await window.api.obtenerPasajeros();
      mostrarPasajeros(pasajeros);
      return;
  }
  
  const resultados = await window.api.buscarPasajeros(searchTerm);
  mostrarPasajeros(resultados);
}

async function buscarVehiculo() {
  const searchTerm = document.getElementById('busquedaVehiculo').value.trim();
  ...
  const resultados = await window.api.buscarVehiculos(searchTerm);
  mostrarVehiculos(resultados);
}

// Cargar datos iniciales
async function cargarDatos() {
  // Cargar conductores si estamos en la página correspondiente
  if (document.getElementById('conductorTableBody')) {
      const conductores = await window.api.obtenerConductores();
      mostrarConductores(conductores);
      
      // Añadir event listener para búsqueda con Enter
      document.getElementById('busquedaConductor')?.addEventListener('keyup', function(e) {
          if (e.key === 'Enter') {
              buscarConductor();
          }
      });
  ...
  // Cargar pasajeros si estamos en la página correspondiente
  if (document.getElementById('pasajeroTableBody')) {
      const pasajeros = await window.api.obtenerPasajeros();
      mostrarPasajeros(pasajeros);
      
      // Añadir event listener para búsqueda con Enter
      document.getElementById('busquedaPasajero')?.addEventListener('keyup', function(e) {
          if (e.key === 'Enter') {
              buscarPasajero();
          }
      });
  ...
  // Cargar vehículos si estamos en la página correspondiente
  if (document.getElementById('vehiculoTableBody')) {
      const vehiculos = await window.api.obtenerVehiculos();
      mostrarVehiculos(vehiculos);
      
      // Añadir event listener para búsqueda con Enter
      document.getElementById('busquedaVehiculo')?.addEventListener('keyup', function(e) {
          if (e.key === 'Enter') {
              buscarVehiculo();
          }
      });
...
window.buscarPasajero = buscarPasajero;
